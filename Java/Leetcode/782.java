//After a swap of columns, two rows that were the same stay the same, and two rows that were different stay different. 
//Since the final state of a chessboard has only two different kinds of rows, 
//there must have originally been only two different kinds of rows.
...
class Solution {
    ...
    public int analyzeCount(Map<Integer, Integer> count, int N) {
        ...
        // If lines aren't opposite k1 ^ k2 should be 11111(N)
        if ((k1 ^ k2) != (1<<N) - 1)
            return -1;
